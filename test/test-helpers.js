...
function makeExpectedCategory(users, category) {
  const user = users.find((singleUser) => singleUser.id === category.user_id || singleUser.id === category['user:id']);
...
function makeExpectedResource(users, categories, resource, comments = []) {
  const user = users.find((singleUser) => singleUser.id === resource.user_id || singleUser.id === resource['user:id']);

  const category = categories.find((singleCategory) => singleCategory.id === resource.category_id || singleCategory.id === resource['category:id']);
...
function makeResourceFixtures() {
  ...
  return {
    testUsers, testCategories, testResources, testComments,
  };
}
